...
from scripts.item_queue_worker import ItemQueueWorker
from stac_fastapi.core.redis_utils import AsyncRedisQueueManager, connect_redis
...
def test_extract_missing_index_key():
    assert ItemQueueWorker._extract_failed_item_ids([{"delete": {"_id": "x|c"}}]) == set()
...
@pytest.mark.asyncio
async def test_flush_dlq_save_failure_keeps_failed_in_pending(queue_manager, dlq_collection):
    col = dlq_collection["id"]
    db = _make_db()
